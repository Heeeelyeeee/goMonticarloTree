...
class Tree:
    ...
    def rolloutRandom(self, s1, gameEnv):
        simOfGame = copy.deepcopy(gameEnv)
        iterationNode = s1
        while True:
            if iterationNode.get_gameInfo().done:
    ...
    def expandingTree(self, s1, iteratorNr, gameEnv, blacksTurn, CNN):
        #getting leafNode:
        ...
        for i in range(0, sizeOfGame*sizeOfGame+1): #all moves 
          ...
          if(simOfGame.valid_moves()[i]==1 ):
            ...
            if CNN:
                ...
                self.backProp(newChild, self.rolloutRandom(newChild,cloneOfGameEnv),startOfExplorationNode)
...
for game in range(0,10):
  ...
  gameTreeRandom = Tree(startNode)
  startNodeRandom = gameTreeRandom.get_start_node()
...
#TreeExploration
  ...
  while not done:
      ...
      timeUsed = 0
      _ = 0
      while timeUsed < 15 and _<explorationRuns:
        if(_%printStatment == 0):
          print("Tree Exploration Iteration: " + str(_))
          
        gameTree.expandingTree(currentNode,_,go_env, False, False)
        timeUsed = time.process_time() - t
        _ += 1
      print("Time Used simulations: " + str())
      #print("PlayingGame")
      ...
      tRandom = time.process_time()
      timeUsedRandom = 0
      _ = 0
      while timeUsedRandom < 15 and _ <explorationRuns:
        if(_%printStatment == 0):
            print("Tree Exploration Iteration: " + str(_))
          
        gameTreeRandom.expandingTree(currentNode,_,go_env, True , True)
        timeUsedRandom = time.process_time() - tRandom
        _ += 1
      #whiteAction =int(input("What should white play?: "))
      #print("White round values:")
      whiteAction = gameTreeRandom.findWorstAction(currentNode, round)
